...
barplot(as.matrix(values_roc_cpt[3:4,]), col = cols, beside = TRUE, ylim = c(0,1.3), yaxt = "n")
legend(12, 1.25, legend = rownames(values_roc_cpt[3:4,]), fill = cols, bty = "n", y.intersp = 0.7)
axis(side = 2, at = seq(0,1,0.2), labels = sprintf("%.2f",seq(0,1,0.2)), las = 2)
mtext(side = 2, text = "AUROC", line = 3, cex = 1)
dev.off()

pdf(file = "CPT1_PR.pdf", width = 9, height = 5)
barplot(as.matrix(values_pr_cpt[3:4,]), col = cols, beside = TRUE, ylim = c(0,1.3), yaxt = "n")
legend(12, 1.25, legend = rownames(values_pr_cpt[3:4,]), fill = cols, bty = "n", y.intersp = 0.7)
axis(side = 2, at = seq(0,1,0.2), labels = sprintf("%.2f",seq(0,1,0.2)), las = 2)
mtext(side = 2, text = "AUPR", line = 3, cex = 1)
dev.off()
