...
output_roc <- "TS2_MSAboost_ROCCurves.pdf"
output_pr <- "TS2_MSAboost_PRCurves.pdf"

data <- read.csv("PHACTboost_vs_MSAboost_TS2.csv")
...
ind <- 1
ind <-  ind + 1
msaboost_roc <- roc(as.numeric(data$MSAboost), as.factor(1 * (y == 1)))
plot(msaboost_roc, col = cols[ind], lwd = 2)
text(0.5, 0.4 - (ind - 1) * 0.07, label = sprintf("MSAboost = %.3f", auc(msaboost_roc)), col = cols[ind], cex = 0.9)
...
ind <- 1
ind <-  ind + 1
msaboost_pr <- pr.curve(scores.class0 = as.numeric(data$MSAboost), weights.class0 = (1 * (y == 1)), curve = TRUE)
plot(msaboost_pr, col = cols[ind], lwd = 2, main = "", auc.main = FALSE)
text(0.5, 0.4 - (ind - 1) * 0.07, label = sprintf("MSAboost = %.3f", (msaboost_pr$auc.integral)), col = cols[ind], cex = 1)
